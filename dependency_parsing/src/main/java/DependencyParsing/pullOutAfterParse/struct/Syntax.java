...
public class Syntax {
    ...
    public static int findMaximalNominalTail(Sentence sent, int t) {
        // traverse amod and nn edges.

        for (int plen=1; plen <= 5; plen++) {
            for (Dep downEdge : sent.gov2childs.get(t)) {
                if ((downEdge.rel.equals("amod") || downEdge.rel.equals("nn")) &&
                        isNominal(sent, downEdge.gov)) {
                    // traverse
                    t = downEdge.child;
                    continue;
                }
...
//		if (d.rel.equals("appos")) return false;
	...
	public static void main(String args[]) throws JsonProcessingException, IOException {
		...
		while ( (line = BasicFileIO.stdin().readLine()) != null) {
			Sentence sent = new Sentence("",-1,line, false);
			U.p(sent);
